        ...
        try:
            ...
            for i, node_name in enumerate(nodes):
                ...
                if reco_detail and reco_detail.hit:
                    ...
                    # 拼接子节点的detail
                    if hasattr(reco_detail, "raw_detail") and isinstance(
                        reco_detail.raw_detail, list
                    ):
                        merged_details.extend(reco_detail.raw_detail)
                    all_failed = False
                else:
            ...
            # 逻辑判断
            logic_result = self._check_logic_condition(logic, node_results)
            if not logic_result:
                # 逻辑判断失败时，返回拼接后的detail数组
                return CustomRecognition.AnalyzeResult(
                    box=[0, 0, 0, 0], detail=merged_details
                )

            # ROI计算
            final_roi = self._process_return_value(return_value, node_results)
            if final_roi:
                # 在detail中返回拼接后的detail数组
                return CustomRecognition.AnalyzeResult(
                    box=final_roi, detail=merged_details
                )
            else:
                # ROI计算失败时，返回拼接后的detail数组
                return CustomRecognition.AnalyzeResult(
                    box=[0, 0, 0, 0], detail=merged_details
                )
            ...
            # 发生异常时，返回包含已执行节点信息的结果
            if "merged_details" in locals():
                return CustomRecognition.AnalyzeResult(
                    box=[0, 0, 0, 0],
                    detail=merged_details,
                )
            else:
                return CustomRecognition.AnalyzeResult(
                    box=[0, 0, 0, 0],
